...
import com.outliner.props.modulecommodityprops.view.TagAdapter;
...
public class TagFlowLayout extends FlowLayout implements TagAdapter.OnDataChangedListener {
    ...
    public void setAllowCancle(boolean allowCancle) {
        isAllowCancle = allowCancle;
    }
    ...
    public boolean performClick() {
        ...
        //2表示为不可选状态
        if (child != null&&mTagAdapter.getSelectState(pos)!=2) {
            doSelect(child, pos);
    ...
    private void doSelect(TagView child, int position) {
        if (mAutoSelectEffect) {
            if (!child.isChecked()&&child.getTagView().isEnabled()) {
                //处理max_select=1的情况
